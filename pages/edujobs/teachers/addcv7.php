...
if (check_if_user_is_teacher($user) && $user->guid==$uguid) { 
	elgg_register_title_button('edujobs','teachers/addportfolio');
	
    $title = elgg_echo('edujobs:cv:add7:simple');
    ...
    elgg_push_breadcrumb(elgg_echo('edujobs:cv:add7:simple'));
	...
	if (!$content) {
		$content = elgg_echo('edujobs:teachers:user:emptycvport');
	} 
