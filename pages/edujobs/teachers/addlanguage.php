...
if (check_if_user_is_teacher($user) && $user->guid==$uguid) { 
    ...
    $title = elgg_echo('edujobs:cv:addlanguage');
    ...
    elgg_push_breadcrumb(elgg_echo('edujobs:cv:add5:simple'), 'edujobs/teachers/addcv5/'.$user->guid); 
    elgg_push_breadcrumb($title);

	if ($guid)	{

		$cv = get_entity($guid);
		$vars = cv_language_prepare_form_vars($cv);	
	}
	else {
		$vars = cv_language_prepare_form_vars();	
	}
	
    // create form
    $form_vars = array('name' => 'educvlangpost', 'enctype' => 'multipart/form-data');
    $content = elgg_view_form('edujobs/cvs/addlanguage', $form_vars, $vars);
